...
type ProjectTab = "overview" | "websites" | "assignments" | "tickets" | "stories" | "work";
...
export default function ProjectDetailPage() {
  ...
  const canManageStories = Boolean(
    user && project && (canAdmin || (user.role === "developer" && project.pic_developer_id === user.id)),
  );
  ...
  const loadTickets = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Gagal memuat tiket Project");
    } finally {
  ...
  const tabs: Array<{ id: ProjectTab; label: string }> = [
    ...
    ...(user.role === "developer" || canAdmin ? [{ id: "stories" as const, label: "User Stories" }] : []),
    ...
    <AppShell title={title}>
      <div className="project-detail-header">
        ...
        <div className="project-summary-strip">
          ...
          <SummaryMetric label="Tiket aktif" value={String(project.active_tickets_count)} detail={`${project.untriaged_tickets_count} belum ditriase`} />
          <SummaryMetric label="Story aktif" value={String(project.active_stories_count)} tone={project.overdue_count ? "down" : undefined} detail={project.overdue_count ? `${project.overdue_count} overdue` : "Tidak ada overdue"} />
        </div>
      </div>

      <nav className="project-tabs" aria-label="Navigasi Project">
        {tabs.map((item) => <button key={item.id} type="button" className={`project-tab ${tab === item.id ? "active" : ""}`} onClick={() => setTab(item.id)}>{item.label}{item.id === "tickets" && project.active_tickets_count > 0 ? <span className="tab-count">{project.active_tickets_count}</span> : null}{item.id === "stories" && project.active_stories_count > 0 ? <span className="tab-count">{project.active_stories_count}</span> : null}</button>)}
      </nav>

      {error ? <ErrorBanner message={error} /> : null}
      {notice ? <SuccessBanner message={notice} /> : null}

      {tab === "overview" ? <OverviewTab project={project} onOpenTab={setTab} showAssignments={canAdmin} showStories={Boolean(canManageStories || canAdmin)} /> : null}
      {tab === "websites" ? <WebsitesTab project={project} canAdmin={canAdmin} onChange={refreshAfterMutation} onAdd={() => setWebsiteOpen(true)} /> : null}
      {tab === "assignments" && canAdmin ? <AssignmentsTab project={project} onSaved={(next, message) => { refreshAfterMutation(next); setNotice(message); }} /> : null}
      {tab === "tickets" ? <TicketsTab tickets={ticketList} loading={ticketsLoading} canManageStories={canManageStories} canCreateTicket={canCreateProjectTicket} onCreateTicket={() => setTicketComposerOpen(true)} onCreateStory={(ticket) => { setStoryFromTicket(ticket); setStoryComposerOpen(true); }} /> : null}
      ...
      {tab === "work" ? <WorkTab project={project} stories={stories} loading={storiesLoading} canManage={canManageStories} onOpenStories={() => setTab("stories")} /> : null}
      ...
      {ticketComposerOpen ? <TicketComposer project={project} onClose={() => setTicketComposerOpen(false)} onSaved={() => { setTicketComposerOpen(false); setNotice("Tiket berhasil dibuat."); void loadTickets(); void loadProject(); }} /> : null}
    </AppShell>
  );
}
  ...
  return (
    <div className="project-overview-grid">
      ...
      <aside className="project-overview-side">
        ...
        <section className="panel"><div className="panel-heading-row"><h3 className="panel-title">Jalur kerja</h3>{showStories ? <button type="button" className="text-link" onClick={() => onOpenTab("stories")}>Buka stories →</button> : <button type="button" className="text-link" onClick={() => onOpenTab("work")}>Buka ringkasan →</button>}</div><div className="workflow-mini"><WorkflowStep label="Intake tiket" value={String(project.active_tickets_count)} /><WorkflowStep label="Story aktif" value={String(project.active_stories_count)} /><WorkflowStep label="Overdue" value={String(project.overdue_count)} danger={project.overdue_count > 0} /></div></section>
      </aside>
    </div>
  );
}
...
function AssignmentsTab({ project, onSaved }: { project: Project; onSaved: (project: Project, message: string) => void }) {
  const [picWeb, setPicWeb] = useState<ProjectRosterCandidate[]>([]);
  const [developers, setDevelopers] = useState<ProjectRosterCandidate[]>([]);
  const [draft, setDraft] = useState<ProjectAssignments>({ pic_web_ids: project.pic_web.map((member) => member.id), pic_developer_id: project.pic_developer_id, developer_ids: project.developers.map((member) => member.id) });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [warning, setWarning] = useState("");
  useEffect(() => { let cancelled = false; Promise.all([projectsApi.roster("pic_web"), projectsApi.roster("developer")]).then(([picWebRows, developerRows]) => { if (!cancelled) { setPicWeb(picWebRows); setDevelopers(developerRows); setLoading(false); } }).catch((err) => { if (!cancelled) { setError(err instanceof ApiError ? err.message : "Gagal memuat roster assignment"); setLoading(false); } }); return () => { cancelled = true; }; }, []);
  const dirty = JSON.stringify(normalizeAssignments(draft)) !== JSON.stringify(normalizeAssignments({ pic_web_ids: project.pic_web.map((member) => member.id), pic_developer_id: project.pic_developer_id, developer_ids: project.developers.map((member) => member.id) }));
  function toggle(field: "pic_web_ids" | "developer_ids", id: string) { setDraft((current) => ({ ...current, [field]: current[field].includes(id) ? current[field].filter((value) => value !== id) : [...current[field], id] })); }
  async function save() { if (!dirty) return; const removed = project.developers.some((member) => !draft.developer_ids.includes(member.id)); if (removed && project.active_stories_count > 0 && !confirm("Sebagian developer memiliki kemungkinan story aktif. Lanjutkan? Histori story tidak dihapus.")) return; setSaving(true); setError(""); setWarning(""); try { const response = await projectsApi.updateAssignments(project.id, draft); setWarning(response.warnings?.join(" ") || ""); onSaved(response, response.warnings?.join(" ") || "Assignment Project disimpan."); } catch (err) { setError(err instanceof ApiError ? err.message : "Gagal menyimpan assignment"); } finally { setSaving(false); } }
  if (loading) return <LoadingState label="Memuat roster assignment…" />;
  return <section className="assignment-workspace"><div className="assignment-intro"><div><span className="eyebrow">Project configuration</span><h3> PIC & Assignment</h3><p className="muted">Atur siapa yang memegang sisi non-teknis, tech lead, dan eksekusi teknis Project ini.</p></div><span className="assignment-rule">Perubahan disimpan sekaligus</span></div>{error ? <ErrorBanner message={error} /> : null}{warning ? <div className="warning-banner">{warning}</div> : null}<div className="assignment-grid"><AssignmentSection title="PIC Web" description="Sisi non-teknis yang memantau masalah dan membuat tiket." count={draft.pic_web_ids.length}><RosterCheckboxList rows={picWeb} selected={draft.pic_web_ids} onToggle={(id) => toggle("pic_web_ids", id)} empty="Belum ada PIC Web aktif." /></AssignmentSection><AssignmentSection title="PIC Developer" description="Tech lead / PIC teknis untuk Project ini. Boleh kosong." count={draft.pic_developer_id ? 1 : 0}><Select value={draft.pic_developer_id ?? ""} onChange={(value) => setDraft((current) => ({ ...current, pic_developer_id: value || null }))} options={[{ value: "", label: "Belum ditentukan" }, ...developers.map((candidate) => ({ value: candidate.id, label: candidate.name }))]} aria-label="PIC Developer" className="block" /><p className="assignment-help">PIC Developer harus memiliki role global Developer. Ia tidak otomatis masuk Developer team.</p></AssignmentSection><AssignmentSection title="Developer Team" description="Tim yang mengerjakan User Story di Project ini." count={draft.developer_ids.length}><RosterCheckboxList rows={developers} selected={draft.developer_ids} onToggle={(id) => toggle("developer_ids", id)} empty="Belum ada developer team." showWorkload /></AssignmentSection></div><div className={`sticky-save-bar ${dirty ? "visible" : ""}`}><span><strong>Perubahan belum disimpan</strong><small>Review assignment sebelum menyimpan.</small></span><div className="row-actions"><button type="button" className="btn" onClick={() => setDraft({ pic_web_ids: project.pic_web.map((member) => member.id), pic_developer_id: project.pic_developer_id, developer_ids: project.developers.map((member) => member.id) })}>Batal</button><button type="button" className="btn btn-primary" disabled={saving} onClick={() => void save()}>{saving ? "Menyimpan…" : "Simpan Perubahan"}</button></div></div></section>;
}

function AssignmentSection({ title, description, count, children }: { title: string; description: string; count: number; children: React.ReactNode }) { return <section className="assignment-section panel"><div className="assignment-section-header"><div><h3>{title}</h3><p className="muted">{description}</p></div><span className="assignment-count">{count}</span></div>{children}</section>; }

function RosterCheckboxList({ rows, selected, onToggle, empty, showWorkload = false }: { rows: ProjectRosterCandidate[]; selected: string[]; onToggle: (id: string) => void; empty: string; showWorkload?: boolean }) { if (!rows.length) return <p className="muted">{empty}</p>; return <div className="roster-list">{rows.map((row) => <label key={row.id} className="roster-option"><input type="checkbox" checked={selected.includes(row.id)} onChange={() => onToggle(row.id)} /><span className="member-avatar">{initials(row.name)}</span><span className="roster-copy"><strong>{row.name}</strong><small>{row.email}</small></span>{showWorkload ? <span className="roster-workload"><b>{row.active_workload}</b> aktif{row.overdue_workload ? <em>{row.overdue_workload} overdue</em> : null}</span> : null}</label>)}</div>; }

function TicketsTab({ tickets, loading, canManageStories, canCreateTicket, onCreateTicket, onCreateStory }: { tickets: Ticket[]; loading: boolean; canManageStories: boolean; canCreateTicket: boolean; onCreateTicket: () => void; onCreateStory: (ticket: Ticket) => void }) {
  if (loading) return <LoadingState label="Memuat tiket Project…" />;
  return <section className="panel"><div className="panel-heading-row"><div><span className="eyebrow">Intake</span><h3 className="panel-title">Tiket Project</h3><p className="muted">Tiket adalah intake. Hanya tiket yang membutuhkan coding yang dikonversi menjadi User Story.</p></div>{canCreateTicket ? <button type="button" className="btn btn-primary" onClick={onCreateTicket}>Buat Tiket</button> : null}</div>{tickets.length === 0 ? <EmptyState title="Belum ada tiket" description="Tiket yang terhubung dengan Project ini akan muncul di sini." /> : <div className="ticket-project-list">{tickets.map((ticket) => <div key={ticket.id} className="ticket-project-row"><div><div className="ticket-title-line"><strong>{ticket.title}</strong><span className={`ticket-status-badge ${ticket.status}`}>{ticket.status.replace(/_/g, " ")}</span></div><p>{ticket.description || "Tidak ada deskripsi."}</p><span className="muted">Dibuat {formatDateTime(ticket.created_at)}{ticket.sla_deadline ? ` · SLA ${formatDateTime(ticket.sla_deadline)}` : ""}</span></div><div className="row-actions">{ticket.user_story_id ? <span className="linked-story-label">Terhubung ke Story</span> : canManageStories ? <button type="button" className="btn btn-sm btn-primary" onClick={() => onCreateStory(ticket)}>Buat User Story</button> : <span className="muted">Belum ditriase</span>}</div></div>)}</div>}</section>;
}
...
function normalizeAssignments(value: ProjectAssignments) { return { pic_web_ids: [...value.pic_web_ids].sort(), pic_developer_id: value.pic_developer_id || null, developer_ids: [...value.developer_ids].sort() }; }
