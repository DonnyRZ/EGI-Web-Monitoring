    ...
    <AppShell title={title}>
      <div className="project-detail-header">
        ...
        <div className="project-summary-strip">
          <SummaryMetric label="Website" value={String(project.websites_count)} />
          <SummaryMetric label="Health" value={healthSummaryLabel(project.health_summary.status)} tone={project.health_summary.status} />
          <SummaryMetric label="Task aktif" value={String(project.active_tickets_count)} />
          {technicalView ? <SummaryMetric label="Pekerjaan teknis aktif" value={String(project.active_stories_count)} tone={project.overdue_count ? "down" : undefined} /> : <SummaryMetric label="Task perlu perhatian" value={String(project.overdue_count)} tone={project.overdue_count ? "down" : undefined} />}
        </div>
      </div>
...
function SummaryMetric({ label, value, detail, tone }: { label: string; value: string; detail?: string; tone?: string }) {
  return <div className="project-summary-metric"><span className="metric-label">{label}</span><strong className={tone ? `summary-value ${tone}` : "summary-value"}>{value}</strong>{detail ? <span className="muted">{detail}</span> : null}</div>;
}
    ...
    <div className="project-overview-grid">
      <section className="panel project-overview-main">
        ...
        <div className="overview-website-list">
          {project.websites.length === 0 ? <div className="inline-empty"><span className="project-status-pill draft">Draft</span><button type="button" className="text-link" onClick={() => onOpenTab("websites")}>Tambahkan Website →</button></div> : project.websites.slice(0, 5).map((website) => <WebsiteMiniRow key={website.id} website={website} />)}
        </div>
      </section>
      ...
      <div ref={modalRef} className="modal project-form-modal project-settings-modal" role="dialog" aria-modal="true" aria-labelledby="project-settings-title" tabIndex={-1} onClick={(event) => event.stopPropagation()}>
        ...
        <form onSubmit={submit}>
          ...
          <fieldset className="project-settings-section">
            ...
            {status === "archived" && project.active_websites_count > 0 ? <div className="warning-banner" role="alert">Website Project Archived tidak lagi tampil di Live Website. Data Website tetap disimpan; nonaktifkan Website pada tab Websites & Monitoring bila monitoring background juga harus dihentikan.</div> : null}
          </fieldset>
